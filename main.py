...
APP_VERSION = '0.5 Alpha - Commit 0001'
THEME = {
    'primary': '#005DAA',
    'primary_dark': '#07355F',
    'primary_mid': '#0079C2',
...
class Hub(tk.Tk):
    def __init__(self):
        ...
        self.title('InSightec Complaint Service Hub'); self.geometry('1320x850'); self.minsize(1160,730); self.configure(bg=THEME['bg'])
    ...
    def build(self):
        ...
        for key in self.get_features():
            name=FEATURES[key][0]; b=tk.Button(self.sidebar,text='  '+name,anchor='w',relief='flat',bg=THEME['primary_dark'],fg='white',activebackground=THEME['primary_mid'],activeforeground='white',font=('Segoe UI',11),command=lambda k=key:self.open_feature(k)); b.pack(fill='x',padx=14,pady=3,ipady=8)
    ...
    def header(self):
        ...
        tk.Label(title,text=' Complaint Service Hub',fg=THEME['text'],bg=THEME['bg'],font=('Segoe UI',22,'bold')).pack(side='left',padx=10)
        ...
        tk.Label(sub,text='Company: '+self.session['company']['display_name']+'   |   Language: '+LANGUAGES.get(self.language_var.get(), self.language_var.get())+'   |   Output: '+self.output_var.get(),bg=THEME['bg'],fg=THEME['text'],font=('Segoe UI',11)).pack(side='left')
        tk.Frame(self.main,bg='#B6D4EE',height=1).pack(fill='x',padx=30,pady=14)
    def home(self):
        for w in self.main.winfo_children(): w.destroy()
        self.header()
        wrap=tk.Frame(self.main,bg=THEME['bg']); wrap.pack(fill='both',expand=True,padx=28,pady=4)
        ...
        tk.Label(wrap,text='Launch Pad',bg=THEME['bg'],fg=THEME['primary_dark'],font=('Segoe UI',17,'bold')).grid(row=0,column=0,columnspan=3,sticky='w',padx=12,pady=(0,4))
        row=1
        for i,key in enumerate(user_feats): self.card(wrap,key,row+i//3,i%3)
        row += (len(user_feats)+2)//3
        if admin_feats:
            sep=tk.Label(wrap,text='Admin Only',bg=THEME['bg'],fg=THEME['admin'],font=('Segoe UI',13,'bold'))
        ...
        status='  ✓ Ready     Version: '+APP_VERSION+'     Master: Loaded     Language: '+LANGUAGES.get(self.language_var.get(), self.language_var.get())+'     Output: '+self.output_var.get()
    ...
    def card(self,parent,key,r,c,admin=False):
        ...
        f=tk.Frame(parent,bg='white',highlightbackground=THEME['line'],highlightthickness=1); f.grid(row=r,column=c,sticky='nsew',padx=12,pady=12)
        ...
        tk.Label(f,text=desc,bg='white',fg=THEME['text'],wraplength=280,justify='left',font=('Segoe UI',10)).pack(anchor='w',padx=24,pady=4)
        badge='Available to all users' if key=='settings' else ('Admin only' if admin else 'User tool')
        tk.Label(f,text=badge,bg=THEME['surface_2'],fg=THEME['primary_dark'],font=('Segoe UI',9,'bold'),padx=8,pady=3).pack(anchor='w',padx=24,pady=(8,0))
